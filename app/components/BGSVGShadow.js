...
const ShapeWithMotion = ({ shape, index, scrollYProgress, mousePosition, onAnimationComplete, totalShapes }) => {
  ...
  const opacity = useTransform(scrollYProgress, [0, 0.4], [1, 0]); // Adjust the fade-out range
...
const BGSVGShadow = ({ onAnimationComplete }) => {
  ...
  const generateShapes = () => {
    ...
    const shapeCount = Math.floor(dimensions.width * dimensions.height / (55000 / ratioAdjustment));
  ...
  return (
    <div ref={sectionRef} className="h-screen w-screen absolute overflow-visible z-[0]">
      <svg width="100%" viewBox="0 0 800 800" height="140%" style={{ overflow: 'visible' }}>
